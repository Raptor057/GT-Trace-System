...
namespace GT.Trace.BomSnapShot.Infra.DataSources
{
    ...
    await _con.QueryFirstAsync<int>("SELECT [dbo].[UfnCheckStatusActiveComponents] (@pointOfUseCode) AS [StatusActiveComponents]", new { pointOfUseCode }).ConfigureAwait(false);
        ...
        public async Task<IEnumerable<pro_production>> LineProductionActiveByPointOfUseCodeAsync(string pointOfUseCode) =>
            await _con.QueryAsync<pro_production>("SELECT TOP (1) PP.* FROM [gtt].[dbo].LineProcessPointsOfUse LPPOU " +
