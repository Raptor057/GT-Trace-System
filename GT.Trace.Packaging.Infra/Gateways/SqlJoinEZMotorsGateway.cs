...
namespace GT.Trace.Packaging.Infra.Gateways
{
    internal class SqlJoinEZMotorsGateway:IJoinEZMotorsGateway
    {
        ...
        public async Task AddJoinEZMotorsAsync(long unitID, string Web, string Current, string Speed, string Date, string Time, string Motor_Number, string PN, string AEM, string Rev)=>
            await _gtt.AddEZJoinMotors(unitID, Web, Current, Speed, Date, Time, Motor_Number, PN, AEM, Rev).ConfigureAwait(false);

        public async Task DelJoinEZMotorsAsync(long unitID)=>
            await _gtt.DelJoinEZMotors(unitID).ConfigureAwait(false);

        public async Task<int> EZMotorDataRegisteredInformationAsync(string Motor_Number, DateTime DateTimeMotor)=>
            await _gtt.EZMotorDataRegisteredInformation(Motor_Number, DateTimeMotor).ConfigureAwait(false);
